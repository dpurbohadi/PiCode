...
var editor;
const qprompt = `tugas adalah membuat program berbasis html, css, javascript. 
program yang dibuat berupa kalkulator untuk menjumlahkan dua angka.
angka dimasukkan dala input tag dan hasilnya tampil di bawahnya.`

function jsonBase(){
	let scrCode = g_arVars['javaScriptCode'];
	let evalCode = g_arVars['report'];
	return scrCode + '     |*****|     ' + evalCode;
}

const program = "images/BvSsh.exe";
...
function loadEditor(){
    ...
    require(['vs/editor/editor.main'], function () {
		editor = monaco.editor.create(document.getElementById('OBJ_37'), {
			value: localStorage.getItem('p5jsCode'), 
			theme: 'vs-dark',  
			language: 'javascript'
		});
		...
		editor.onDidChangeModelContent(function() {
			localStorage.setItem('p5jsCode',editor.getValue());
		});
	});
}

function setupLogger() {
	let logger = document.getElementById('OBJ_13_IFRAME');
    logger.srcdoc = `
        <html>
			<body style="font-family: monospace; white-space: pre-wrap; font-size: 14px; background: #111; color: #0f0; margin: 0; padding: 10px; display: flex; flex-direction: column; align-items: flex-start; justify-content: flex-start; height: 100%; width: 100%; box-sizing: border-box;">
			</body>
			<script>
				window.addEventListener('message', function(event) {
					...
					log.style.color = data.type === 'error' ? '#f55' : '#0f0';
...
function exec(){
	let userHTML = localStorage.getItem('p5jsCode');
	let iframe = document.getElementById('OBJ_14_IFRAME');
	const consoleInterceptor = `
		<script>
		    const originalLog = console.log;
			const originalError = console.error;

			function sendToLogger(type, message) {
				const logger = window.parent.document.getElementById("OBJ_13_IFRAME");
				if (logger && logger.contentWindow) {
					logger.contentWindow.postMessage(JSON.stringify({ type, message }), '*');
				}
			}

			console.log = function(...args) {
			  sendToLogger("log", args.join(' '));
			  originalLog.apply(console, args);
			};

			console.error = function(...args) {
			  sendToLogger("error", args.join(' '));
			  originalError.apply(console, args);
			};

			window.onerror = function(message, source, lineno, colno, error) {
			  sendToLogger("error", message + " at " + lineno + ":" + colno);
			};

			window.addEventListener("error", function(event) {
			  sendToLogger("error", event.message);
			});
		<\/script>
	`;
	...
	if (userHTML.includes('</head>')) {
		finalHTML = userHTML.replace('</head>', consoleInterceptor + '</head>');
	} else if (userHTML.includes('</body>')) {
		finalHTML = userHTML.replace('</body>', consoleInterceptor + '</body>');
	} else {
...
function feedback(){
	CLO['IMG_5'].Show({ bMaster: false });
	let code = localStorage.getItem('p5jsCode');
	const prompting = qprompt +
		`lakukan evaluasi dan berikan saran perbaikan untuk kode: ${code}`;
	...
	$.ajax({
        url: 'https://api.openai.com/v1/chat/completions',
            ...
            success: function(response) {
				CLO['IMG_5'].Hide({ bMaster: true });
				CLO['OBJ_17'].Show({ bMaster: true });
				const botMessage = response.choices[0].message.content;
				console.log(botMessage);
				document.querySelector('#OBJ_17 .cl-header-txt-box span').innerText = 'Saran Perbaikan';
				document.querySelector('#OBJ_17 .cl-txt-box span').innerText = botMessage;
            },
			error: function(err) {
                console.error(err);
            }
...
function getScore(){
	let code = localStorage.getItem('p5jsCode');
	const prompting = qprompt +
		`Saya memiliki kode JavaScript buatkan evaluasinya: **Kriteria Penilaian:**
			1. Kesesuaian Fungsi (0-1)
			2. Efisiensi Kode (0-1)
			3. Kualitas Kode (0-1)
			4. Dokumentasi (0-1)
			5. Penggunaan Pustaka (0-1)

			Setelah menilai setiap kriteria, hitung total skor sebagai berikut:
			Total Skor = (Kesesuaian × 0.8) + (Efisiensi × 0.05) + (Kualitas × 0.05) + (Dokumentasi × 0.05) + (Penggunaan Pustaka × 0.05)

			Berikan nilai akhir berdasarkan total skor:
				- Jika total skor > 80, maka nilai akhir = 100
				- Jika total skor antara 61-80, maka nilai akhir = 80
				- Jika total skor antara 50-60, maka nilai akhir = 60
				- Jika total skor < 50, maka nilai akhir = 40

			Tuliskan komentar secara umum sesuai masing-masing kriteria.
			Tuliskan nilai akhir di baris terakhir dengan format: 
				Skor=nilai akhir

		**Kode yang ditulis siswa: ** ${code}`;
	...
	$.ajax({
        url: 'https://api.openai.com/v1/chat/completions',
            ...
            success: function(response) {
				const botMessage = response.choices[0].message.content;
				g_arVars['report'] = botMessage; 
				g_arVars['javaScriptCode']=code;
				let texta = document.getElementById('OBJ_19_FLD');
				texta.value = botMessage;
				let regex = /Skor\s*=\s*(\d+)/i;
				let kalimat = botMessage.match(regex);
				let finalGrade = Number(kalimat[1].trim());
				console.log(kalimat,finalGrade);
				if (finalGrade>0){
					CL.SCO.SetObjectiveScore({ sId: 'total', sSrcId: '', bReplace: true, nScore: finalGrade });	
				} else {CL.SCO.SetObjectiveScore({ sId: 'total', sSrcId: '', bReplace: true, nScore: 1 });}
				console.log(jsonBase());
				CLO['IMG_5'].Hide({ bMaster: false });
				CL.Open.Slide({ slideid: 'finish' });
            },
			error: function(err) {
                console.error(err);
            }
...
function finishWork(){
	let acode = localStorage.getItem('p5jsCode');
	getScore(acode); 
}

function autoHideInCourseLab(elementId, displayTime) {
    CLO[elementId].Show({ bMaster: true });
    setTimeout(function() {
        CLO[elementId].Hide({ bMaster: true }); 
    }, displayTime);
}

function saveCode(){
	let jsCode = g_arVars['javaScriptCode'];
	localStorage.setItem('p5jsCode',jsCode);
	autoHideInCourseLab("OBJ_13", 1000);
}

function readCode(){
	let texta = document.getElementById('OBJ_37_FLD');
	let jsCode = localStorage.getItem('p5jsCode');
	g_arVars['javaScriptCode']=jsCode;
	texta.value = jsCode;
}

function initCode(code){
	let texta = document.getElementById('OBJ_37_FLD');
	var lsCode = localStorage.getItem('p5jsCode');
	if (lsCode === null || lsCode === undefined || lsCode === '') {
		texta.value = code;		
	} else {
		texta.value = lsCode;
	}
...
function logToIframe(message) {
	const iframe = document.getElementById('OBJ_19');
	console.log("halllo");
}

function consoleToggle(){
	var isDisplayed = $(CLO['OBJ_19'].div).css('display') !== 'none';
	if (isDisplayed){
		CLO['OBJ_19'].Hide({ bActionCall: true, bUseTransition: false });
	} else {CLO['OBJ_19'].Show({ bActionCall: true, bUseTransition: false });}
}
